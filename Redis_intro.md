...
TTL (Time To Live) in Redis defines how long a key is allowed to exist before it is automatically deleted. When a TTL is set on a key, Redis associates an expiration timestamp with that key. Once the current time passes this timestamp, this key is considered expired and is removed from memory. TTL is a fundamental feature in Redis system design because it prevents stale data, controls memory usage, and enables time-based behaviors such as caching, session expiration, and rate limiting. Without TTL, Redis memory would grow indefinitely, making it unsuitable for high-traffic, long-running systems.
- Internally, Redis handle expiration using a combination of lazy deletion and active expiration. In lazy deletion, Redis checks whether a key is expired only when the key is accessed; if it has expired, Redis deletes it at that moment.
- In active expiration, Redis periodically samples a subset of keys with TTLs and proactively removes those that have expired. This hybrid approach allows Redis to efficiently manage expired keys without scanning the entire keyspaces, which would be too expensive. This design ensures good performance even when millions of keys have expiration times.